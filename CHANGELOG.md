...
### Fixed
- **Startup screen drawn with the wrong orientation**: the boot animation ("TJ-56-654" logo) was rendered before the configured `display_orientation` was applied, so with orientation 2 (180°) it appeared upside down until the next screen. The configuration is now loaded and the rotation applied **before** the startup animation.
...
### Security
- **`/api/status` now requires authentication**: the endpoint reveals the WiFi SSID and local IP, so it now sits behind HTTP Basic auth like the other sensitive endpoints. The only remaining public endpoints are the home page `/` and `/api/time`.
...
### Added
- **Weather icon**: the weather screen now shows a 16×16 icon based on the Open-Meteo `weathercode` (sun, cloud/fog, rain, snow, thunder).

### Changed
- **Admin password hidden**: the password field in `/config` now uses a masked `password` input instead of plain text.

### Notes
- Major version bump to 2.0.0 reflects a visible UI/UX improvement.
...
### Added
- **Configurable admin credentials**: username and password for `/update` and ArduinoOTA can now be changed from `/config` or via `/api/config`.
  - Defaults remain `admin` / `admin`.
  - Applied after save + reboot.
  - Safe EEPROM migration: devices updating from older firmware keep default credentials if the fields were never written.
